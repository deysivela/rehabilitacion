...
const Pacientes = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const pacientesOrdenados = pacientesRes.data.sort((a, b) => b.Idpac - a.Idpac);
  ...
  const pacientesFiltrados = pacientes.filter(paciente => {
    const cumpleFiltro = 
      `${paciente.Nombre_pac} ${paciente.Appaterno_pac || ""} ${paciente.Apmaterno_pac || ""} ${paciente.Ci_pac}`
      .toLowerCase()
              ...
              {pacientesFiltrados.length > 0 ? (
                pacientesFiltrados.map((paciente) => {
                  ...
                  return (
                    <tr key={paciente.Idpac}>
                      ...
                      <td>{paciente.Genero_pac === 'F' ? 'Femenino' : 'Masculino'}</td>
                      <td>{edad.texto}</td>
                      <td>{paciente.Telefono_pac || '-'}</td>
                      <td>{paciente.Diagnostico ? `${paciente.Diagnostico.substring(0, 20)}...` : '-'}</td>
                      <td>
                      ...
                      <td className="acciones">
                        ...
                        <button
                          ...
                          onClick={() => registrarDiagnostico(paciente.Idpac)}
                        >
                          <FaPlusCircle />
                        </button>
                        <Link
                ...
                <tr>
                  <td colSpan="8" className="no-results">
                    No se encontraron pacientes {filtro || rangoEdad.min || rangoEdad.max ? 'con esos filtros' : ''}
                  </td>
                </tr>
              )}
            </tbody>
            ...
            <div className="modal-body">
              <div className="detalle-row">
                <div className="detalle-col">
                  ...
                  <p><strong>Nombre completo:</strong> {pacienteSeleccionado.Nombre_pac} {pacienteSeleccionado.Appaterno_pac} {pacienteSeleccionado.Apmaterno_pac}</p>
                  <p><strong>CI:</strong> {pacienteSeleccionado.Ci_pac || 'No especificado'}</p>
                  <p><strong>Género:</strong> {pacienteSeleccionado.Genero_pac === 'F' ? 'Femenino' : 'Masculino'}</p>
                  <p><strong>Fecha de nacimiento:</strong> {new Date(pacienteSeleccionado.Fnaci_pac).toLocaleDateString()}</p>
                  <p><strong>Edad:</strong> {calcularEdadCompleta(pacienteSeleccionado.Fnaci_pac).texto}</p>
                </div>
                
                <div className="detalle-col">
                  <h3>Información de Contacto</h3>
                  <p><strong>Teléfono:</strong> {pacienteSeleccionado.Telefono_pac || 'No especificado'}</p>
                  <p><strong>Dirección:</strong> {pacienteSeleccionado.Direccion_pac || 'No especificado'}</p>
                </div>
              </div>
              ...
              <div className="detalle-row">
                <div className="detalle-col">
                  ...
                  <p><strong>Diagnóstico Inicial:</strong> {pacienteSeleccionado.Diagnostico || 'No especificado'}</p>
                </div>
                ...
                <div className="detalle-col">
                  ...
                  {pacienteSeleccionado.Tienediscapacidad?.toLowerCase() === 'sí' && (
                    <>
